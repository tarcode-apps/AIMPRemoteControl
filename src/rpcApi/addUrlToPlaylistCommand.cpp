...
void rpcapi::AddUrlToPlaylistCommand::Register(IEndpointRouteBuilder &endpoints)
{
	endpoints.MapRpc("AddURLToPlaylist", [core = FCore, &idManager = FIdManager](const nlohmann::json &params) -> nlohmann::json
			{
		...
		const HRESULT result = RunOnMainThread(core, [&] { return AddFilesByPlaylistId(core, idManager, playlistId, {url}); });
