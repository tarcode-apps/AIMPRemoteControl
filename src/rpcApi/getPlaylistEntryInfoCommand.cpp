...
void rpcapi::GetPlaylistEntryInfoCommand::Register(IEndpointRouteBuilder &endpoints)
{
	endpoints.MapRpc("GetPlaylistEntryInfo", [core = FCore, &idManager = FIdManager](const nlohmann::json &params) -> nlohmann::json
			{
		...
		const nlohmann::json result = RunOnMainThread(core, [&]() -> nlohmann::json
		{
			...
			nlohmann::json info = nlohmann::json::object();
			{
				...
				const PlaylistEntryIds ids{trackId, idManager.PlaylistGetOrGeneratePluginId(GetPlaylistAIMPId(playlist))};
				for (const char *field : InfoFields)
					info[field] = PlaylistEntryField(field, ctx, ids);
			}
